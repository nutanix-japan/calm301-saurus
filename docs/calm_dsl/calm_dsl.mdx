...
1.  Go to the Calm Marketplace to launch the provisioning of the DevWorkstation.
    ...
    ![](images/MPDevWorkstation.png)

1.  **Important: Select the Credentials tab and enter desired User/Pass**

    ![](images/Creds.png)

1.  Enter the name of Application DevWorkstation-INITIALS and fill out the form with the requested information

    ![](images/DevLaunch.png)

1.  Press Create
2.  While waiting review the audit log to see packages being deployed. The blueprint automatically installs several utilities along with Calm DSL
3.  Once the application is **running** SSH to the DevWorkstation
...
1.  Start the virtual environment and connect to Prism Central
2.  Change to the **calm-dsl** directory from home
3.  Run the following to switch to the virtual environment. This will enable the virtual environment for Calm DSL
    
    ```bash
    cd calm-dsl
    source venv/bin/activate
    ```
4.  **Optional:** This has already been done through the blueprint launch. Once you SSH into the DevWorkstation we can setup the
    connection to Prism Central by running the following command:

    ```bash
    calm init dsl 
    ```

5.  Verify the current config settings by running
...
2.  Back in github there should be a new verion under the **history** of blueprint.py with the changed memory
